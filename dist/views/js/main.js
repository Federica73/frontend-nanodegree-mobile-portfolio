...
function generator(adj, noun) {
    var adjectives = getAdj(adj), nouns = getNoun(noun), randomAdjective = parseInt(Math.random() * adjectives.length), randomNoun = parseInt(Math.random() * nouns.length), name = "The " + adjectives[randomAdjective].capitalize() + " " + nouns[randomNoun].capitalize();
    return name;
}

function randomName() {
    var randomNumberAdj = parseInt(Math.random() * adjectives.length), randomNumberNoun = parseInt(Math.random() * nouns.length);
    return generator(adjectives[randomNumberAdj], nouns[randomNumberNoun]);
}

function logAverageFrame(times) {
    var i, numberOfEntries = times.length, sum = 0;
    for (i = numberOfEntries - 1; i > numberOfEntries - 11; i--) sum += times[i].duration;
...
var adjectives = [ "dark", "color", "whimsical", "shiny", "noise", "apocalyptic", "insulting", "praise", "scientific" ], nouns = [ "animals", "everyday", "fantasy", "gross", "horror", "jewelry", "places", "scifi" ], selectRandomMeat = function() {
    ...
    return randomCrust;
}, ingredientItemizer = function(string) {
    return "<li>" + string + "</li>";
}, makeRandomPizza = function() {
    for (var pizza = "", numberOfMeats = Math.floor(4 * Math.random()), numberOfNonMeats = Math.floor(3 * Math.random()), numberOfCheeses = Math.floor(2 * Math.random()), i = 0; numberOfMeats > i; i++) pizza += ingredientItemizer(selectRandomMeat());
    for (var j = 0; numberOfNonMeats > j; j++) pizza += ingredientItemizer(selectRandomNonMeat());
    for (var k = 0; numberOfCheeses > k; k++) pizza += ingredientItemizer(selectRandomCheese());
    return pizza += ingredientItemizer(selectRandomSauce()), pizza += ingredientItemizer(selectRandomCrust());
}, pizzaElementGenerator = function(i) {
    var pizzaContainer, pizzaImageContainer, pizzaImage, pizzaDescriptionContainer, pizzaName, ul;
    return pizzaContainer = document.createElement("div"), pizzaImageContainer = document.createElement("div"), 
    pizzaImage = document.createElement("img"), pizzaDescriptionContainer = document.createElement("div"), 
    pizzaContainer.classList.add("randomPizzaContainer"), pizzaContainer.style.width = "33.33%", 
    pizzaContainer.style.height = "325px", pizzaContainer.id = "pizza" + i, pizzaImageContainer.classList.add("col-md-6"), 
    pizzaImage.src = "images/pizza.png", pizzaImage.classList.add("img-responsive"), 
    pizzaImageContainer.appendChild(pizzaImage), pizzaContainer.appendChild(pizzaImageContainer), 
    pizzaDescriptionContainer.classList.add("col-md-6"), pizzaName = document.createElement("h4"), 
    pizzaName.innerHTML = randomName(), pizzaDescriptionContainer.appendChild(pizzaName), 
    ul = document.createElement("ul"), ul.innerHTML = makeRandomPizza(), pizzaDescriptionContainer.appendChild(ul), 
...
var pizzasDiv = document.getElementById("randomPizzas"), i;

for (i = 2; 100 > i; i++) pizzasDiv.appendChild(pizzaElementGenerator(i));
...
window.addEventListener("scroll", updatePositions), document.addEventListener("DOMContentLoaded", function() {
    ...
    window.items = document.getElementsByClassName("mover"), updatePositions();
});